...
- **A new law can cost tens of GB in the checker, so cap the check before you run it.** The checker reduces `Nat` in UNARY, so a closed law whose arithmetic reaches a large literal does not fail fast: it allocates. Observed twice independently — a closed law at `2^24` scale, and a candidate law of round 26 whose REDUCED proof sat at 22 GB for 20 minutes before it was killed (it was dropped, never committed; that run is what reaped another session's background jobs). Write the quantity in `U32` words rather than `Nat` literals, and run a new-law check under a limit (round 26 settled on 6 GB and 180 s) instead of discovering the cost by exhausting the host. A full `PROOF.bend` over the committed laws is a different matter and peaks near 6 GB. When the proof is slow or hangs and you do not know WHICH law is responsible, `python3 scripts/bisect-laws.py` answers exactly that: it checks each law alone in a probe project under a wall-clock timeout and a memory cap and reports OK, RED, TIMEOUT or INCONCLUSIVE per law. Nothing referenced it before 2026-09-26, which is why round 26 diagnosed its 22 GB law by killing the run instead. **Serialize heavy checker runs on the host, and never run one bare.** A cap protects the host from ONE process and says nothing about the sum of two: on 2026-09-26 a full `PROOF.bend` started beside a review round's law-mutation batch already at 8.4 GB, and `MemAvailable` on this 30 GB box fell to 1.1 GB, where the OOM killer would have taken whichever run it preferred. The fix is the mechanism that round 31 had already built and the second agent was not using: take an exclusive `flock` on a shared heavy lock and wait for about 12 GB available before starting, holding the lock for the whole run (`/data/tmp/review_R31/heavy.lock` while that round lives, else `/data/tmp/toon_bend_heavy.lock`). When a collision does happen, stop YOUR OWN newer run — never another agent's process — and tell the other session which window its timings were contended in.
...
| the spec's homes | `python3 scripts/arch-lint.py` (every spec clause of S1–S5, S8, S9 is named in a `docs/PROPOSED_ARCHITECTURE.md` table, each numeric quantity has a plan row, each S6 clause a carrier). It is a Phase 2 gate that keeps biting in Phase 4/5, because every review round adds clauses: nothing referenced it, so nobody ran it, and on 2026-09-26 its first run in this phase found THIRTEEN clauses implemented and tagged in `port/` with no row in the module map (`S4.230`, `S4.231`, `S9.148`–`S9.155`, `S1.156`–`S1.158`). The code had not drifted; the document had |
