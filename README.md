...
### The Solution
`toon_bend` is built with the *porting-to-bend2* method: the original is pinned and RUN as an oracle (never read while implementing), its behavior is written down as a numbered specification (716 clauses), the Bend code is written from that specification, and it is judged against 1074 captured cases on four executor lanes. Two equivalences are kept apart:
...
| --- | --- |
| Byte-for-byte parity, golden-tested | stdout, stderr and the exit code match the pinned original byte for byte on 1074 captured cases; see **Status** below for the lanes and the commit each line was produced on |
...
| Proved properties | 425 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 25 hold for every input (the first failure ends a pass, in strict mode a line after the complete root and a repeated key that cannot merge are reported, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, equal estimates print `No token difference`, the kill-switch closes every fast path, with key folding off a fold attempt never folds, under the kill-switch no fold context is lean, and the parallel number pre-pass changes when a number is printed, never what is printed, and is closed by the kill-switch, the pass renders a number with the printer of the side it runs on, and --decode is the side that asks for the JSON printer); 400 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 36 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`): 25 in one complete run, round 14's five in a second, two from an author-side hunt in a third, round 15's two in a fourth and round 16's two in a fifth (both KILLED, verdict STRONG; the line is pasted in `docs/PORT_STATE.md`), each by the law written for it. Adding laws can only add ways for a mutant to fail, so an earlier kill stands under the larger law set; that is an inference from five runs, not one run of all 36 |
...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS 1071/1071 on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, inside one GREEN `scripts/port-doctor.sh` run on the goldens re-captured from `694d73b` (proof, lanes, board, floor, kill-switch parity; the table is pasted in `docs/PORT_STATE.md`, which also says what changed in `port/` after it: comments only). `scripts/conform.sh` also passes 1071/1071 on c-1t with `TOON_SPEC=1`. **That four-lane line is the tree of `8d32b6e` and its 1071-case corpus.** On the current tree (`5c3ac52`, the R16-1 fix and its three new cases, 1074 cases), `scripts/conform.sh` passes **1074/1074 on c-1t, on c-8t, on js, and on c-1t with `TOON_SPEC=1`**, and `bend PROOF.bend` prints `All terms check.` with 425 laws — but **the interpreter lane has not been run on it**, so no four-lane claim exists for `5c3ac52`. MANIFEST: 3222 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself: 2 repeats in that run, 3 in an earlier dedicated one). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 425 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
...
| --- | --- | --- | --- | --- |
| `toon_bend` (this repo) | Bend 2: native C, JavaScript, interpreter | 1074 captured cases x 4 lanes, differential fuzzing against the oracle | laws checked by Bend | port of `toon_rust` 0.2.4 at `694d73b` |
