...
| Proved properties | 374 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 14 hold for every input (the first failure ends a pass, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, the kill-switch closes every fast path); 360 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 25 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`) |
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 374 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
