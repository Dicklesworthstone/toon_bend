...
### The Solution
`toon_bend` is built with the *porting-to-bend2* method: the original is pinned and RUN as an oracle (never read while implementing), its behavior is written down as a numbered specification (716 clauses), the Bend code is written from that specification, and it is judged against 1076 captured cases on four executor lanes. Two equivalences are kept apart:
...
| --- | --- |
| Byte-for-byte parity, golden-tested | stdout, stderr and the exit code match the pinned original byte for byte on 1076 captured cases; see **Status** below for the lanes and the commit each line was produced on |
...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS **1076/1076** on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, on the tree of `cbdfba6` (2026-09-23, local 16:28 to 18:15; later commits change laws, harness scripts and documents only; the line is pasted in `docs/PORT_STATE.md`). `scripts/conform.sh` also passes 1076/1076 on c-1t with `TOON_SPEC=1`. MANIFEST: 3228 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 594 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
...
- **Verdict:** HOLD, not SHIP (`docs/PORT_REPORT.md`). What holds it: the convergence rule needs two clean non-author review rounds in a row on a frozen tree (fewer than three BEHAVIOR findings each: a difference from the original, a wrong proof verdict or a lying gate; since 2026-09-23 a surviving hand mutant is a law-coverage finding and a refuted claim a document finding, both repaired before SHIP, neither resetting the count, `docs/PORT_STATE.md` "Owner decisions"), and no round has been clean yet (`scripts/converge.sh docs/PORT_STATE.md`). Every divergence is ACCEPTED with a scoped contract or RESOLVED (`docs/DISCREPANCIES.md`); no known bug of the original is reproduced.
...
- **Work, COUNTED on the current tree** (instructions, `valgrind --tool=cachegrind --cache-sim=no`, `--threads 1`; the
  ...
  first): the port executes **6.09×** the pinned original's instructions (geometric mean; encode 7.36×, decode 5.03×;
  10.83× against the original built at `opt-level=3`), from 6.83× on the tree of `2c33e64`. Best cell `apache_builds`
  decode 3.31×, worst `canada` encode 13.22× (111080 doubles through a software binary64: Bend has no `f64`). Tree of
  `f3314a3`, 2026-09-23, `perf/evidence/COUNTED.e2e-corpus.f3314a3.json`. Instruction counts see no cache miss and no
...
**Counted, not measured: the allocation levers of 2026-09-23** (EXP-013, 018, 019, 021, 022, 024, 025, 026, 027, 029, 031, 032; `perf/NEGATIVE-EVIDENCE.md` NE-018, NE-022 to NE-025, NE-028 to NE-032, NE-035 and NE-036; the other commits between the two trees change laws and ledgers only). These are INSTRUCTION COUNTS (`valgrind --tool=cachegrind --cache-sim=no`, `--threads 1`), not times: deterministic on this shared host (two runs of one binary differ by 80 instructions in 7.3 billion, where wall-clock cv was 10-50%), blind to cache misses and allocation latency, and therefore never called MEASURED and never used to meet a gate written in time. The session-start binary (`722991e`) against the tree of `f3314a3`, stdout identical in every cell, inputs from the e2e corpus (`perf/e2e/`); one file per cell in `perf/evidence/` (`COUNTED.<input>.<mode>.f3314a3.json`, both counts and both stdout hashes; the files named `.faaccac.` hold the comparison before EXP-031 and 032, `.cbdfba6.` the one before EXP-029, those without a commit in their name the one before EXP-025 to 027):

| input | mode | instructions before | after | ratio |
| --- | --- | --- | --- | --- |
| `gsoc_2018` (3.2 MB, long texts) | `--encode` | 7,325,873,588 | 2,813,787,258 | 2.604× fewer |
| `gsoc_2018` | `--decode` | 7,356,818,902 | 3,318,526,350 | 2.217× fewer |
| `flights_200k` (9.4 MB, numbers in rows) | `--encode` | 39,442,102,850 | 28,340,783,307 | 1.392× fewer |
| `flights_200k` | `--decode` | 32,743,595,074 | 27,259,753,592 | 1.201× fewer |
| `canada` (2.1 MB, coordinates) | `--encode` | 12,726,037,094 | 10,930,199,115 | 1.164× fewer |
| `canada` | `--decode` | 16,671,049,526 | 12,830,013,817 | 1.299× fewer |

What they remove: in the emitted C every list cell built on top of another passes through `rfc_wrap` (a refcount cell) and is later dropped, and the port built about 10.7 cells per input byte; the levers stop reversing a whole value to read its last character (EXP-013), stop building text that is never read (EXP-018), stop copying the input (EXP-019), build the output once instead of three times (EXP-021), stop reversing trimmed values twice (EXP-022) walk a quoted literal once instead of twice (EXP-024), scan the decoded text in the order the UTF-8 decoder already holds it (EXP-025), stop keeping every string value for a numeric-like test that now only reads it (EXP-026), take a plain byte inside a JSON string without the reader's whole per-byte dispatch (EXP-027), detect a repeated key in a small JSON object by walking its chain instead of the key table (EXP-029), stop keeping the input bytes alive through a decoding (EXP-031), and read the input bytes once when encoding, the UTF-8 check and the JSON reader in one walk (EXP-032). EXP-031 and 032 each failed one of their own guard cells by 0.8 to 1.8% on code they did not change: clang inlines the UTF-8 loop differently in each build (`perf/NEGATIVE-EVIDENCE.md`, the preamble's rule on counted deltas across builds). A CPU capture on a quiet host is what would make any of them a MEASURED ratio.
...
| --- | --- | --- | --- | --- |
| `toon_bend` (this repo) | Bend 2: native C, JavaScript, interpreter | 1076 captured cases x 4 lanes, differential fuzzing against the oracle | laws checked by Bend | port of `toon_rust` 0.2.4 at `694d73b` |
