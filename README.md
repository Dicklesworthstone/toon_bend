...
| --- | --- |
| Byte-for-byte parity, golden-tested | stdout, stderr and the exit code match the pinned original byte for byte on 1071 captured cases; see **Status** below for the lanes and the commit each line was produced on |
...
| Exact numbers without an `f64` | Bend has no binary64: the port carries a software binary64 over big naturals and reproduces three different number algorithms of the original (serde_json's non-round-trip float path, Rust's `Display`, correctly rounded token parsing), bit for bit on every number of the corpus and of the differential runs |
| Proved properties | 383 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 14 hold for every input (the first failure ends a pass, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, the kill-switch closes every fast path); 369 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 34 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`): 25 in one complete run, round 14's five in a second, two from an author-side hunt in a third and round 15's two in a fourth, each by the law written for it. Adding laws can only add ways for a mutant to fail, so an earlier kill stands under the larger law set; that is an inference from four runs, not one run of all 34 |
...
Every line names its command, its lanes and its commit. Dates: the gate run of local 2026-09-21 (its rows are stamped 2026-09-22 UTC), unless a line gives its own. Bend: 2.0.16 at `15ae0c8`. Original: `toon_rust` `7c1d6e4` (`toon 0.2.4`), re-pinned on 2026-09-22 from `f955c67` when the upstream fix of candidate C-10 landed (a bracket inside a quoted value was read as an array header, so `toon` could not decode its own encoding of a real document; PLAN §2); the lines below that name `f955c67` were produced against that earlier pin.

- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS 1071/1071 on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, inside one GREEN `scripts/port-doctor.sh` run on this commit's `port/` and `goldens/` (proof, lanes, board, floor, kill-switch parity; the table is pasted in `docs/PORT_STATE.md`, which also shows why every input that run gated is unchanged at the newest commit). `scripts/conform.sh` also passes 1071/1071 on the three compiled lanes with `TOON_SPEC=1`. MANIFEST: 3213 hashes, captured from `./oracle/toon`; floor STABLE (the original against itself: 2 repeats in that run, 3 in an earlier dedicated one). The two native lanes are ONE sequential execution under two labels: the port places no bang and no parallel let, so the runtime never starts a worker pool and `--threads N` changes nothing (2 OS threads at `--threads` 1, 8 and 64, measured by round 10 and re-measured); `c-8t` adds no evidence beyond `c-1t` today and is kept because it would catch a parallel twin the day one is added.
- **Outside the corpus:** seeded differential fuzzing against the original (`scripts/diff-fuzz.py`: mutated corpus documents, generated documents through every option, command lines, numbers under both settings of the kill-switch, expansion, keys chosen to collide in the port's hash, inputs large in one dimension with a time verdict) and the descriptor states the harness cannot express (`scripts/stdio-probe.py`: inherited offsets, sockets, closed, read-only and full standard streams, descriptor paths, message-oriented sockets). NON-AUTHOR review rounds (round 6 onward; every report is in `docs/reviews/`, every round has a row and a paragraph in `docs/PORT_STATE.md`) compared well over a million executions with the original. Round 7 found the two behavioral HIGH findings of this port (stdin used to be re-opened by path: wrong bytes for a file at an offset, a failure for a socket; repaired by the stdin effect), round 10 one about the repository (cited commits that did not build, because `.gitignore` hid two source files); since round 9 no round has found a difference in conversion content, and their findings are about descriptor states, the launcher and these documents. Every finding is repaired or registered.
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 383 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
...
- **Verdict:** HOLD, not SHIP (`docs/PORT_REPORT.md`). What holds it: the convergence rule needs two clean non-author review rounds in a row (fewer than three findings each), and no round has been clean yet (`scripts/converge.sh docs/PORT_STATE.md`). Every divergence is ACCEPTED with a scoped contract or RESOLVED by a repair (`docs/DISCREPANCIES.md`).
...
| --- | --- | --- | --- | --- |
| `toon_bend` (this repo) | Bend 2: native C, JavaScript, interpreter | 1071 captured cases x 4 lanes, differential fuzzing against the oracle | laws checked by Bend | bug-compatible port of `toon_rust` 0.2.4 |
