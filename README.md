...
**The port against the original on ordinary inputs: no claim.** Two attempts of eight captures each were ALL REFUSED by the cv gate on the loaded host (`perf/evidence/INCUMBENT.*.json` holds the second attempt: arms between 3 and 61 percent cv; the original finishes most of these inputs in under 10 ms, which a shared host cannot time to 5 percent), so this README states no ratio. The refused medians, for orientation only and not as evidence: encode the 1500-row table: 74.2 ms against 7.5 ms; decode it: 51.5 ms against 22.2 ms; encode 24000 integers: 65.5 ms against 8.4 ms; encode 9000 one-decimal numbers: 168.7 ms against 4.4 ms; encode 7000 strings: 42.6 ms against 5.5 ms; encode 20000 doubles: 1397.1 ms against 11.8 ms; encode 5000 scientific-notation numbers: 1509.4 ms against 5.7 ms; `--version`: 2.3 ms against 1.9 ms. What is structural and will not change: Bend has no `f64`, so every non-integer number goes through big naturals (DISC-013); every input byte is a heap cell of a checked state machine; and the native runtime reserves 8 TiB of address space and uses 47 to 70 bytes of memory per input byte (DISC-011).
