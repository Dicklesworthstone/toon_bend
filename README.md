...
| Proved properties | 381 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 14 hold for every input (the first failure ends a pass, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, the kill-switch closes every fast path); 367 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 30 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`): 25 in one complete run and round 14's five in a run of their own, each by the law written for it. Adding laws can only add ways for a mutant to fail, so the 25 stay killed under the larger law set; that is an inference from two runs, not one run of all 30 |
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 381 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
