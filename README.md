...
### The Solution
`toon_bend` is built with the *porting-to-bend2* method: the original is pinned and RUN as an oracle (never read while implementing), its behavior is written down as a numbered specification (716 clauses), the Bend code is written from that specification, and it is judged against 1077 captured cases on four executor lanes. Two equivalences are kept apart:
...
| --- | --- |
| Byte-for-byte parity, golden-tested | stdout, stderr and the exit code match the pinned original byte for byte on 1077 captured cases; see **Status** below for the lanes and the commit each line was produced on |
...
| Proved properties | 604 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 41 hold for every input (the first failure ends a pass, in strict mode a line after the complete root and a repeated key that cannot merge are reported, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, equal estimates print `No token difference`, the kill-switch closes every fast path, with key folding off a fold attempt never folds, under the kill-switch no fold context is lean, and the parallel number pre-pass changes when a number is printed, never what is printed, and is closed by the kill-switch, the pass renders a number with the printer of the side it runs on, and --decode is the side that asks for the JSON printer, and the shortest-digit loop over words runs exactly when the kill-switch is open and the state fits, and the argv scanner's gate for a word that begins with '-' takes the plain arm or the short-option arm as its verdict says, and the JSON reader's string arm with its fast path closed is the reader's step); 563 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 43 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`), each by the law written for it, in TWO runs: the first 36 in one run on 2026-09-23, `{"laws_in_proof": 233, "laws_sha256_12": "c55f99164d90", "reduced": true, "mutants": 36, "killed": 36, "survived": [], "not_evidence": [], "verdict": "STRONG"}`, and the seven round 20 added (M38 to M44) in one run on 2026-09-24, `{"laws_in_proof": 359, "laws_sha256_12": "8ec9282df760", "reduced": true, "mutants": 7, "killed": 7, "survived": [], "not_evidence": [], "verdict": "STRONG"}`. That the 36 still die against the laws added since rests on the inference that adding laws can only add ways for a mutant to fail; one run of all 43 is what would remove it. The `laws_sha256_12` digest is over the sorted names of the laws actually compiled into the proof, so the verdict carries its own provenance |
...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS **1077/1077** on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, on the tree of `5856d1b` (2026-09-24, local 06:03 to 07:36; the commits after it change documents only; the line is pasted in `docs/PORT_STATE.md`). `scripts/conform.sh` also passes 1077/1077 on c-1t with `TOON_SPEC=1` (2026-09-24). MANIFEST: 3231 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 604 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
...
| --- | --- | --- | --- | --- |
| `toon_bend` (this repo) | Bend 2: native C, JavaScript, interpreter | 1077 captured cases x 4 lanes, differential fuzzing against the oracle | laws checked by Bend | port of `toon_rust` 0.2.4 at `694d73b` |
