...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS **1076/1076** on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, on the tree of `9c9039e` (2026-09-24, local 02:47 to 04:29; the commits after it change documents only; the line is pasted in `docs/PORT_STATE.md`). `scripts/conform.sh` also passes 1076/1076 on c-1t with `TOON_SPEC=1`. MANIFEST: 3228 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
