...
### The Solution
`toon_bend` is built with the *porting-to-bend2* method: the original is pinned and RUN as an oracle (never read while implementing), its behavior is written down as a numbered specification (716 clauses), the Bend code is written from that specification, and it is judged against 1086 captured cases on four executor lanes. Two equivalences are kept apart:
...
| --- | --- |
| Byte-for-byte parity, golden-tested | stdout, stderr and the exit code match the pinned original byte for byte on 1086 captured cases; see **Status** below for the lanes and the commit each line was produced on |
...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS **1086/1086** on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, on the tree of `82eceb3` (2026-09-25, local 02:25 to 04:00; the commits after it change documents only; the line is pasted in `docs/PORT_STATE.md`). `scripts/conform.sh` also passes 1086/1086 on c-1t with `TOON_SPEC=1` (2026-09-25). MANIFEST: 3258 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 637 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
...
| --- | --- | --- | --- | --- |
| `toon_bend` (this repo) | Bend 2: native C, JavaScript, interpreter | 1086 captured cases x 4 lanes, differential fuzzing against the oracle | laws checked by Bend | port of `toon_rust` 0.2.4 at `694d73b` |
