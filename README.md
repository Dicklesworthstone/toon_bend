...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS 1071/1071 on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, inside one GREEN `scripts/port-doctor.sh` run on the goldens re-captured from `694d73b` (proof, lanes, board, floor, kill-switch parity; the table is pasted in `docs/PORT_STATE.md`, which also says what changed in `port/` after it: comments only). `scripts/conform.sh` also passes 1071/1071 on c-1t with `TOON_SPEC=1`. MANIFEST: 3213 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself: 2 repeats in that run, 3 in an earlier dedicated one). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
...
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 417 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
