...
| Proved properties | 383 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 14 hold for every input (the first failure ends a pass, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, the kill-switch closes every fast path); 369 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 32 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`): 25 in one complete run, round 14's five in a second and two from an author-side hunt in a third, each by the law written for it. Adding laws can only add ways for a mutant to fail, so earlier kills stand under the larger law set; that is an inference from three runs, not one run of all 32 |
...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS 1065/1065 on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, inside one GREEN `scripts/port-doctor.sh` run on the tree of commit `2dee063` (proof, lanes, board, floor, kill-switch parity; the table is pasted in `docs/PORT_STATE.md`, which also shows why every input that run gated is unchanged at the newest commit). `scripts/conform.sh` also passes 1065/1065 on the three compiled lanes with `TOON_SPEC=1`. MANIFEST: 3195 hashes, captured from `./oracle/toon`; floor STABLE (the original against itself: 2 repeats in that run, 3 in an earlier dedicated one). The two native lanes are ONE sequential execution under two labels: the port places no bang and no parallel let, so the runtime never starts a worker pool and `--threads N` changes nothing (2 OS threads at `--threads` 1, 8 and 64, measured by round 10 and re-measured); `c-8t` adds no evidence beyond `c-1t` today and is kept because it would catch a parallel twin the day one is added.
