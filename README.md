...
### The Solution
`toon_bend` is built with the *porting-to-bend2* method: the original is pinned and RUN as an oracle (never read while implementing), its behavior is written down as a numbered specification (716 clauses), the Bend code is written from that specification, and it is judged against 1127 captured cases on four executor lanes. Two equivalences are kept apart:
...
| --- | --- |
| Byte-for-byte parity, golden-tested | stdout, stderr and the exit code match the pinned original byte for byte on 1127 captured cases; see **Status** below for the lanes and the commit each line was produced on |
...
| Proved properties | 703 laws checked by Bend (`All terms check.`, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16): 57 hold for every input (the first failure ends a pass, in strict mode a line after the complete root and a repeated key that cannot merge are reported, lenient mode never reports a body check, `--encode` beats every extension, no expansion step runs at depth 256, equal estimates print `No token difference`, the kill-switch closes every fast path, with key folding off a fold attempt never folds, under the kill-switch no fold context is lean, and the parallel number pre-pass changes when a number is printed, never what is printed, and is closed by the kill-switch, the pass renders a number with the printer of the side it runs on, and --decode is the side that asks for the JSON printer, and the shortest-digit loop over words runs exactly when the kill-switch is open and the state fits, and the argv scanner's gate for a word that begins with '-' takes the plain arm or the short-option arm as its verdict says, and the JSON reader's string arm with its fast path closed is the reader's step, and escaping a byte of any of the six escape classes and unescaping the result leaves the unescaper in the state the raw byte would have produced, for any text so far and any continuation, and the same for every arm of the JSON writer's escape table read back through the JSON reader's own machine, for any reader state and either setting of the kill-switch); 646 are closed instances computed by the checker itself, 295 of them captured goldens restated as laws. 131 hand-written mutants of the code the laws speak about are all killed (`scripts/hand-mutants.py`), each by a law that pins it; every run is pasted with its JSON line in `docs/PORT_STATE.md` ("law admission"). That the earlier mutants still die against the laws added after their runs rests on the inference that adding laws can only add ways for a mutant to fail. The `laws_sha256_12` digest is over the sorted names of the laws actually compiled into the proof, so the verdict carries its own provenance |
...
- **Golden-tested, every lane:** `scripts/lanes.sh goldens/cases.tsv goldens port/main.bend --threads 8` → PASS **1127/1127** on the interpreter, the native binary at 1 and at 8 threads, and the JavaScript build, on the tree of `e19d8b5` (2026-09-26, local 09:12 to 10:49; the commits after it change documents only; the line is pasted in `docs/PORT_STATE.md`). `scripts/conform.sh` also passes 1127/1127 on c-1t with `TOON_SPEC=1` (2026-09-26). MANIFEST: 3381 hashes, captured from `./oracle/toon` (sha256 `821287ea…`); floor STABLE (the original against itself). Since EXP-007 (`0131342`) the encoder's number pre-pass is a parallel let: above `--threads 1` the runtime runs it on a worker pool, so `c-8t` is a genuinely parallel lane on every `--encode` case; decoding and every other path stay sequential, and no bang is placed.
- **Outside the corpus:** seeded differential fuzzing against the original (`scripts/diff-fuzz.py`: mutated corpus documents, generated documents through every option, command lines, numbers under both settings of the kill-switch, expansion, keys chosen to collide in the port's hash, inputs large in one dimension with a time verdict) and the descriptor states the harness cannot express (`scripts/stdio-probe.py`: inherited offsets, sockets, closed, read-only and full standard streams, descriptor paths, message-oriented sockets). NON-AUTHOR review rounds (round 6 onward; every report is in `docs/reviews/`, every round has a row and a paragraph in `docs/PORT_STATE.md`) compared well over a million executions with the original. Round 7 found the two behavioral HIGH findings of this port (stdin used to be re-opened by path: wrong bytes for a file at an offset, a failure for a socket; repaired by the stdin effect), round 10 one about the repository (cited commits that did not build, because `.gitignore` hid two source files); across rounds 9 to 30 exactly one difference in conversion content was found, round 16's R16-1 (a JSON number the port printed as 0 with exit 0 where the original exits 1), fixed in `5c3ac52`; the other rounds' findings are about descriptor states, the launcher, the review gates, hand mutants the corpus missed, and these documents. Every finding is repaired or registered. Rounds 16 to 30 are non-author reviews of the re-pinned code (none clean yet under the counting rule; see **Verdict**); the author's own checks of it: the stdio probe (27 rows: 13 SAME, 13 KNOWN, 1 FIXED, 0 NEW), and every S10 row judged against the TOON specification and the reference implementation (spec Amendment A3).
- **Proved:** `bun /tmp/bend/bend2/main.ts port/PROOF.bend` → `All terms check.`, 703 laws, unsafe 0 = 0 `@unsafe` + 0 template instances, bend 2.0.16.
...
- **Verdict:** HOLD, not SHIP (`docs/PORT_REPORT.md`). What holds it: the convergence rule needs two clean non-author review rounds in a row on a frozen tree (rounds 20 to 31 counted by the porting method's recommended rule, under which a gate that misreads and a hand mutant surviving the corpus and the proof also made a round dirty; from round 32, by the owner's decision of 2026-09-26, only a HIGH or MEDIUM difference between the port and the original, or a wrong proof verdict, does, and every other finding must still be repaired; `docs/PORT_STATE.md` "Owner decisions"), and no round has been clean yet (`scripts/converge.sh docs/PORT_STATE.md`): rounds 32 and 33 are the first two counted under the new rule. Every divergence is ACCEPTED with a scoped contract or RESOLVED (`docs/DISCREPANCIES.md`); no known bug of the original is reproduced.
...
# Decode TOON back to JSON (the pinned original prints the same bytes)
...
| --- | --- | --- | --- | --- |
| `toon_bend` (this repo) | Bend 2: native C, JavaScript, interpreter | 1127 captured cases x 4 lanes, differential fuzzing against the oracle | laws checked by Bend | port of `toon_rust` 0.2.4 at `694d73b` |
