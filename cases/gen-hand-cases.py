...
# round 21 (R21-1): a later row lists the header's keys in another order and holds an object or an array under one of them;
# the array goes to list form (the corpus had no reordered row with a nested value)
...
def _proposed_e():
    ...
    # round 25 (R25-3): four decoder and three argv behaviours whose hand mutants passed the whole corpus and the whole proof
    dec("toonedge_list_item_ws_before_nested_array", "[2]:\n  -  [2]: 1,2\n  - \t[1]: 3\n", cls="toon-edge", note="S4.222: White_Space before the nested array's `[` in a list item is skipped (D7)")
    dec("toonerr_colon_before_delimiter_row", "[1]{a,b}:\n  x:1,y:2\n", cls="toon-error", note="S4.214: the FIRST colon comes before the first delimiter -> not a row (D25)")
    dec("toonedge_delimiter_before_colon_row", "[1]{a,b,c}:\n  1,x:y,z\n", cls="toon-edge", note="S4.214: the FIRST delimiter comes before the first colon -> a row (D26)")
    dec("toonerr_list_line_after_full_table", "[1]{a,b}:\n  1,2\n  - 3,4\n", cls="toon-error", note="S9.152: a `- ` line after a full root table is content after the root, not a surplus row (D5)")
    case("usage_negative_exponent_dot_word", ["--indent", "-1e5.3"], None, "usage", "S1.156: '-1e5.3' is no number word (a '.' after the exponent) -> unknown -1 (C3)")
    case("usage_negative_trailing_exponent_word", ["--indent", "-1e"], None, "usage", "S1.156: '-1e' is no number word (an exponent with no digit) -> unknown -1 (C5)")
    case("usage_flatten_depth_negative_value", ["--flatten-depth", "-1"], None, "usage", "S1.156: --flatten-depth takes a negative number word as its value -> invalid digit (C6)")
    # round 26 (R26-3): ten behaviours whose hand mutants passed the whole corpus and the whole proof
    dec("toonerr_deeper_row_not_a_row", "[2]{a,b}:\n  1,2\n    3,4\n", cls="toon-error", note="S4.212: a line deeper than the row depth is no row -> 1 of 2 rows (N6)")
    dec("toonerr_deeper_item_not_an_item", "[2]:\n  - 1\n    - 2\n", cls="toon-error", note="S4.215: a line deeper than the item depth is no item -> 1 of 2 items (N7)")
    dec("toonerr_deeper_line_after_full_table", "[1]{a,b}:\n  1,2\n    3,4\n", cls="toon-error", note="S9.152: a deeper line after a full root table is content after the root, not a surplus row (N1)")
    dec("toonerr_blank_before_surplus_row", "[2]{a}:\n  1\n\n  2\n  3\n", cls="toon-error", note="S4.213: the blank-line check comes before the surplus check (N4)")
    dec("toonerr_nesting_limit_at_tabular_body", "".join("  " * i + "k:\n" for i in range(125)) + "  " * 125 + "t[1]{a}:\n" + "  " * 126 + "1\n", cls="toon-error", note="S9.153: a tabular body counts two containers -> the 128th opens at line 126 (N10)")
    dec("toonerr_nesting_limit_at_empty_item", "".join("  " * i + "k:\n" for i in range(125)) + "  " * 125 + "t[1]:\n" + "  " * 126 + "-\n", cls="toon-error", note="S9.153: an empty list-item object is a container -> line 127 (N11)")
    case("usage_negative_leading_dot_word", ["--indent", "-.5"], None, "usage", "S1.156: '-.5' is no number word (a '.' may not be first) -> unknown -. (C1)")
    case("usage_negative_two_dots_word", ["--indent", "-1.2.3"], None, "usage", "S1.156: '-1.2.3' is no number word (at most one '.') -> unknown -1 (C2)")
    enc("jsonerr_exponent_overflow_column_2147483648", "[1e21474836480]", cls="json-error", note="S2.14: the exponent overflows at its 10th digit -> column 13 (J1)")
    enc("jsonerr_exponent_overflow_column_2147483640", "[1e21474836400]", cls="json-error", note="S2.14: 2147483640 fits, the next digit overflows -> column 14 (J2)")
    # round 27 (R27-3): seven behaviours whose hand mutants passed the whole corpus and the whole proof
    enc("enc_fold_flatten_depth_100_on_120_chain", "{\"k\":" * 120 + "1" + "}" * 120, ["--key-folding", "safe", "--flatten-depth", "100"], note="S4.79: a three-digit budget folds exactly 100 segments, never unlimited (E11)")
    enc("jsonerr_ef_bb_not_a_bom", b"\xef\xbb\x80", cls="json-error", note="S2.3: EF BB followed by another byte is no byte order mark -> expected value at column 1 (C6)")
    dec("toonedge_inline_array_counts_one_level", "".join("  " * i + "k:\n" for i in range(125)) + "  " * 125 + "t[1]: 1\n", cls="toon-edge", note="S9.153: an inline array weighs one container, so 125 objects and it fit (N18)")
    dec("toonerr_nesting_limit_at_item_first_field", "".join("  " * i + "k:\n" for i in range(125)) + "  " * 125 + "t[1]:\n" + "  " * 126 + "- a: 1\n", cls="toon-error", note="S9.153: an item object with its first field on the hyphen line is a container -> line 127 (N19)")
    enc("enc_fold_partial_keeps_enclosing_prefix", "{\"x.a.b.c.d\":1,\"x\":{\"z\":0,\"a\":{\"b\":{\"c\":{\"d\":2},\"e\":3}}}}", ["--key-folding", "safe"], note="S4.71: under a partial fold the prefix is the enclosing path, so c.d is not folded into a collision (E13)")
    case("usage_indent_negative_exponent_word", ["--indent", "-1e5", "--encode"], "{\"a\":1}", "usage", "S1.156: '-1e5' is a number word -> invalid value for --indent (C11)", "json")
    enc("encnum_zero_mantissa_exponent_overflow_by_zero", "[0e21474836470]", cls="enc-number", note="S2.14: an exponent overflow on a zero mantissa is not out of range -> 0 (J5)")
    # Round 28 (R28-3): the corpus gaps of six mutants, each input the reviewer's own, captured from the original.
    enc("jsonerr_overlong_utf8_3byte_last", b'"\xe0\x9f\xbf"', cls="json-error", note="S2.2: E0 9F BF, the LAST 3-byte overlong (U+07FF), is invalid UTF-8 (U3)")
    dec("toonerr_overlong_utf8_4byte_last", b"a: \xf0\x8f\xbf\xbf", cls="toon-error", note="S2.2: F0 8F BF BF, the last 4-byte overlong (U+FFFF), is invalid UTF-8 (U4)")
    dec("toonerr_c0_is_no_continuation_byte", b"a: \xc3\xc0", cls="toon-error", note="S2.2: C0 after a 2-byte lead is no continuation byte -> invalid UTF-8 (U6)")
    enc("encnum_shortest_upper_end_odd", "[38933209638946775]", cls="enc-number", note="S4.133: an odd significand excludes the interval's upper end -> 38933209638946776 (F2)")
    dec("decfmt_shortest_upper_end_odd", "a: 38933209638946775\n", cls="dec-number", note="S4.172: the same number through the JSON writer -> 38933209638946776 (F2)")
    enc("encnum_shortest_lower_end_odd", "[36899328138587787]", cls="enc-number", note="S4.133: an odd significand excludes the interval's lower end -> 36899328138587784 (F3)")
    case("usage_indent_i64_min", ["--encode", "--indent=-9223372036854775808"], "{}", "usage", "S1.31: i64::MIN parses and is then not in 0..=16 (Ce)", "json")
    # Round 29 (R29-3): three corpus gaps, then three behaviours a law pinned but no case did (N4, K1, U11).
    dec("toonedge_bracket_only_pipe_is_key", "a[|]:\n", cls="toon-edge", note="S2.135: bracket content that is only a delimiter mark is no length, so `a[|]` is a key (H5)")
    dec("toonedge_bracket_only_tab_is_key", "a[\t]{x}:\n", cls="toon-edge", note="S2.135: a tab alone in brackets is no length either (H5)")
    dec("toonerr_char_after_quoted_key", "\"k\"x1]: 5\n", cls="toon-error", note="S2.130: only `[` or `:` may follow a quoted key's closing quote -> missing colon (H6)")
    enc("jsonerr_surrogate_last_dfff", b'"\xed\xbf\xbf"', cls="json-error", note="S2.2: ED BF BF encodes U+DFFF, the last surrogate -> invalid UTF-8 (U10)")
    ...
    dec("toonerr_sibling_bare_dash", "l[1]:\n  - a: 1\n    -\n", cls="toon-error", note="S4.225 (amended round 25): a bare '-' sibling is over-indented -> Validation error at line 3")
