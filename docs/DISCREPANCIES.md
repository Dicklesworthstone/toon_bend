...
- Original behavior (run by round 11, R11-2; read sizes re-traced by the author with `strace`): Rust's `read_to_end` asks for 32 bytes first, then for the buffer's spare capacity, which doubles: 32, 32, 64, 128, … So (a) on a packet-mode pipe (`pipe2(O_DIRECT)`) or a `SOCK_SEQPACKET` socket, one message LONGER than 32 bytes is cut by the kernel at the first read: `JSON error: Failed to parse JSON: EOF while parsing a list at line 1 column 32`, exit 1; (b) a regular file opened `O_RDONLY|O_DIRECT` as descriptor 0 fails with `Failed to read stdin: Invalid argument (os error 22)`, exit 1 (an unaligned 32-byte read)
- Port behavior: `File.read_bytes(f, 1048576)`: one read of 1 MiB into the runtime's buffer takes the whole message, and that buffer satisfies O_DIRECT: the document converts, exit 0, on the native binary, through the launcher and on the JavaScript build. Messages of at most 32 bytes, stream sockets, pipes, files, terminals: SAME
...
- Kill-switch: not applicable
- Affected cases: none expressible; `scripts/stdio-probe.py` rows "stdin is a SEQPACKET socket, one message of 31 bytes" (SAME) and "… of 41 bytes" (KNOWN)
...
- Approver: the repository owner, 2026-09-20, by the delegation quoted under DISC-001 ("You decide on everything. I approve whatever you want to do."); the author's ruling follows under Resolution
- Resolution: ACCEPTED. Scoped contract: only a descriptor 0 on which the size or the alignment of a read is observable (packet-mode pipes, SEQPACKET sockets with a message above 32 bytes, O_DIRECT files); there the port converts where the original reports an error. This is the port behaving better than the original, which by this project's rule is still a divergence and is registered, not silent
