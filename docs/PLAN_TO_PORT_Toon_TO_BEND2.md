...
| commit / version | `7c1d6e439feab93bf816e99180e7261ec118dacb` (2026-09-22, clean tree) / `toon 0.2.4`. RE-PINNED on 2026-09-22 from `f955c67015d45330935b3ae4d62178d2bb44a481` (2026-09-12), which every capture and measurement before that date used: the owner ordered every bug found in either repository fixed, and the e2e benchmark (`perf/e2e/`) found that the original could not decode its own encoding of a real document (C-10, S10.83). `7c1d6e4` is `f955c67` plus that one decoder fix, two dependency bumps (`clap`/`clap_builder` 4.6.6 → 4.6.7, `clap_derive` 4.6.4 → 4.6.7; the corpus shows no usage-text change) and changes outside `src/` (fixtures formatting, CI, ignore files). Against the 1071 goldens of `f955c67` the new build differed in exactly the 7 cases of S10.83/S10.84/S10.89 and in no other; upstream `https://github.com/Dicklesworthstone/toon_rust`; behavior-bearing locked dependencies: `serde_json 1.0.151` (parse, error text, string escaping), `zmij 1.0.23` (JSON float text), `indexmap 2.14.0` (key order, duplicates), `clap 4.6.7` / `clap_builder 4.6.7` (argv, usage text, exit 2) |
