...
| every perf claim has a pin, cv ≤ 5%, identical sha | the MEASURED captures named below; one NO_EVIDENCE; ten files in `perf/evidence/` carry `REFUSED_CV` and are claimed nowhere (the five INCUMBENT captures, the one-lever captures of EXP-001 and EXP-002 with two arm files each, and the folding capture that is the NO_EVIDENCE one); NO row is admitted to the WIN ledger (NE-001, 002, 003 and 005 are provisional, NE-004 is NO_EVIDENCE) | `perf/evidence/`, `perf/PERF-LEDGER.md`, `perf/NEGATIVE-EVIDENCE.md` |
