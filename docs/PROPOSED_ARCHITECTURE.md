...
| `dec.array.open`, `dec.fits.obj`, `dec.fields.depth`, `dec.x.depth` | S4.230, S4.231, S9.150, S9.155 | spec | consumes the line list | ADDED Phase 4/5 from review rounds: the decoder refuses to open a 128th object or array, and path expansion is held to the same 127 levels. The clause tags are on these defs in `port/decode.bend`; the limits are pinned by `expand_depth_127_accept`, `expand_depth_128_reject` and `expand_segments_253_not_ok` |
| `dec.field.one`, `dec.hdr.h`, `dec.dup.msg`, `dec.nest.msg`, `dec.next.num` | S9.148, S9.149, S9.151, S9.152, S9.153, S9.154 | spec | consumes the scanned line | ADDED Phase 4/5 from review rounds: the decoder's message texts for an empty field list, a header shape, a repeated key, a nesting jump and a trailing line. Tagged on these defs in `port/decode.bend` |
| `cli.tok.neg`, `cli.indent.msg` | S1.156, S1.157, S1.158 | spec | consumes argv | ADDED Phase 4/5 from review rounds: a `-1` word is an unexpected argument rather than a value, and the help text states the indent range and the delimiter words. Tagged on these defs in `port/cli.bend` |
...
| A9 | §1 the shell reads stdin with the method's recipe, `File.open("/dev/stdin", "r")`; §2 "no custom effects" | the shell's ONE custom effect `Stdin.open() -> IO(Result<…, File>)`: `stdin_open.c` answers `dup(0)` as a Base `File`, `stdin_open.js` answers descriptor 0; everything after it (`File.read_bytes` in 1 MiB chunks, strict UTF-8) is unchanged | round 7 (non-author), R7-1..R7-3 and R7-8: a path re-open is a new open file description: a regular file was read from byte 0 whatever the inherited offset (wrong bytes, exit 0), a socket could not be opened (every Node.js child process), a file unreadable by path failed, `O_NONBLOCK` was lost. The effect has a twin per lane, so no lane is lost (the interpreter runs the `.js` twin). Evidence: `scripts/stdio-probe.py` (DISC-012, DISC-009 RESOLVED); `lanes.sh` on every lane |
| A10 | A8's buckets: a list per trie leaf | an AA tree per trie leaf, ordered by `T.str_cmp` (`T.KS` for the key set, `J.KB` with a value per key, `D.XV`'s `XBNode`), and the JSON reader no longer rewrites the entry chain for a repeated key: the last value goes to a map (`over: J.KM` in the object frame) that is applied once when the object closes | round 7, R7-4 and R7-5: 16000 keys chosen to collide in the 16 hash bits took 21 s (expansion 99 s), 16000 keys each repeated once 50 s; the original's maps are SipHash-keyed and its repeats are O(1). Now each costs a logarithm of the bucket: `scripts/diff-fuzz.py scale` (hostile keys, a time verdict) and `collide`; laws `str_cmp_orders`, `ks_bucket_is_balanced`, `ks_bucket_members`, `kt_collision_*`, `golden_*hash_collision*`, `golden_encstr_duplicate_keys*` |
| A11 | §3 two JSON writers (A and B, two escape tables); the JSON reader's number path is serde_json's non-round-trip algorithm (`F.serde_value`, S4.100–S4.122); the decoder has no nesting limit and keeps repeated keys | 2026-09-22, re-pin to `toon_rust` `694d73b` (spec Amendment A1): ONE writer `J.w`/`J.write_ln` with one escape table; `J.num.value` reads through `F.token_value` (correctly rounded; the serde path and `F.fixed1` were removed as dead code); the decoder machine counts open containers against 127 (`D.nest.over`, S4.230) and merges or rejects repeated keys (`D.dups`, S9.151); expansion is held to 127 levels (`D.decode.x.depth`, S4.231) | the owner's order that every bug be fixed, not reproduced; the laws about removed defs were restated on the live ones (`serde_short_42`, `serde_short_zero`, `serde_decimal_3_7` on `J.num.value`; `writer_b_keeps_*` on `J.w.chars`) or replaced where the behavior itself changed (`json_exponent_at_k17` → `json_plain_at_k21`, `json_exponent_at_k22`; `expand_cap_is_256_accept` → `expand_segments_253_hits_nesting_cap`, `expand_depth_127_accept`, `expand_depth_128_reject`, `expand_segments_253_not_ok`) |
