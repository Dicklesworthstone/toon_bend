...
**The same effect in NE-035 and NE-036** (found and closed under `toon_bend-2ul`): their guard failures, +0.80% on `gsoc_2018 --encode` and +1.0 to 1.8% on four decode cells, are the two cells the corrected paragraph above cites, on code neither lever changed.
...
- Provenance: as NE-022; base = the binary of the EXP-029 code (`ac6ff70`), `--threads 1`
...
- Binding (in the parked code): the quantified law that the fused walk IS the two walks (withdrawn with the code on 2026-09-24) (`run.u(bytes, u, st, spec) == RU{utf8.bytes(bytes, u), run(bytes, st, spec)}` for every input and both states, by induction on the bytes with a split on the reader's mode), five closed `run_pure` goldens from the pinned original (an invalid byte after a JSON error, a truncated sequence after one, an invalid byte in a key under `--stats`, `--stats` after a mark, two marks); in a reduced proof two hand mutants were killed (the decoder step dropped from one arm: the fused walk's law (withdrawn); the reader's error preferred to the UTF-8 verdict: `encode_invalid_utf8_after_json_error`)
...
### NE-035 — decode mode stops handing the input bytes on to the encode arm (EXP-031)   [2026-09-23 | COUNTED_WIN on the primary gate, encode guard FAILED, kept as a trade-off]
- Program / def: `port/cli.bend` / `convert` decides the mode first (`convert.mode`, `o.decode`, `o.stats`); decode reads the text alone (`convert.text` no longer takes the bytes); encode is `ac6ff70`'s path under a new name (`convert.enc`). Not behind the switch: no value changes, only which def holds the bytes
- Provenance: as NE-022; base = the binary of `ac6ff70` (the EXP-029 code), `--threads 1`
- Counted (cachegrind Ir), stdout identical in every cell. The gate, on inputs NE-034's count never ran: `semanticscholar.toon --decode` 13,793,961,050 → 11,286,330,337, **18.18% fewer** (gate 5%); `twitter.toon` 855,605,291 → 705,217,456 (17.58% fewer); `citm_catalog.toon` 1,649,910,384 → 1,472,365,733 (10.76% fewer); `openapi_github.toon` 15,584,536,523 → 13,014,689,484 (16.49% fewer). The encode guard (not worse by more than 0.5%): `flights_200k` 28,679,609,592 → 28,748,656,810 (+0.24%), `canada` 11,015,737,330 → 11,031,495,105 (+0.14%), **`gsoc_2018` 2,913,444,694 → 2,936,739,179 (+0.80%): the guard FAILED on this cell**
- Where the encode cost is (cg_annotate, gsoc encode): the UTF-8 walk of the encode arm counts 186,358,580 against 163,064,066 in the base (about 7 more per byte); the ownership in the emitted C is unchanged (381 keeps, 365 takes, 77 peeks in both), the constructor numbering is not. A second shape (a separate loop for decode, so each loop has one call site, bound by a quantified law) was counted and was WORSE everywhere (`gsoc_2018 --encode` 3,043,229,797, +4.5%; `flights_200k --encode` +1.34%; the decode cells 1.0-1.8 points less good): not kept, and its law was withdrawn with it before any commit
- Why kept although the guard failed: the guard was written to catch an encode regression and it caught one of 0.80% on one cell of three; the lever takes 10.8-18.2% off every decode cell counted, four of them out of sample. This is the author's judgement against the author's own precommitted guard, stated here so it is not silent; revert `convert.mode` to undo it
- Binding: three closed `run_pure` goldens from the pinned original (`decode_invalid_utf8_after_toon_error`, `decode_truncated_utf8_at_the_end`, `decode_stats_after_mark_multibyte`) beside the existing decode UTF-8 and mark goldens; a hand mutant (decode started with the verdict-only decoder, which builds no text) is killed by `decode_stats_after_mark_multibyte` in a reduced proof
- Correctness evidence: conform c-1t 1076/1076 with `TOON_SPEC` unset and set; `scripts/diff-fuzz.py` seed 3131: mutate and docs 1500 inputs each, 0 differences
- Killing metric: a cv-gated CPU capture on a quiet host showing the encode cost larger than the decode gain on the corpus
- **Do-not-retry unless:** (a win with a cost, kept) — bead `toon_bend-1yf` for the encode arm's 7 instructions per byte; revert if the owner prefers the guard
- Tally: W1/L0/N0 (counted, guard failed)
- Agent: Claude (session ef481f9c, 2026-09-23)

### NE-036 — on top of EXP-031, encoding reads the input bytes once (EXP-032, NE-034's fused walk retried)   [2026-09-23 | NEGATIVE(reverted) 2026-09-24: a lane difference]
- Program / def: `port/json.bend` / `RU`, `run.u`, `RR`, `read.u.fin`, `read.u`; `port/cli.bend` / `convert.mode`'s encode arm, `convert.enc` taking the reader's pair (S2.2, S2.4, S2.45, S8.7, S9.10)
- Provenance: as NE-022; base = the binary of the EXP-031 code (commit of NE-035), `--threads 1`
- Counted against EXP-031 (cachegrind Ir), stdout identical in every cell. Gate: `semanticscholar.json --encode` 9,431,730,088 → 9,115,988,884, **3.35% fewer** (gate 1.5%; never counted by EXP-030 or EXP-031). Encode guards: `gsoc_2018` 2,936,739,179 → 2,813,787,258 (4.19% fewer), `flights_200k` 28,748,656,810 → 28,340,783,307 (1.42% fewer), `canada` 11,031,495,105 → 10,930,199,115 (0.92% fewer). **Decode guard (not worse by more than 0.5%) FAILED**: `semanticscholar.toon` 11,286,330,337 → 11,492,568,790 (+1.83%), `twitter.toon` 705,217,456 → 718,199,230 (+1.84%), `citm_catalog.toon` 1,472,365,733 → 1,487,685,277 (+1.04%), `openapi_github.toon` 13,014,689,484 → 13,229,328,991 (+1.65%)
- The decode path's Bend code is the same in both builds (the lever adds two types and four defs to `json.bend` and changes only the encode arm); the decode difference is clang inlining the UTF-8 loop into the worklist function once it has a single call site (the header's COUNTED-resolution paragraph, measured per function on `twitter.toon`)
- The stack EXP-031 + EXP-032 against `ac6ff70` (the base of both), stdout identical: `gsoc_2018` encode 3.42% fewer, decode 19.56% fewer; `flights_200k` encode 1.18%, decode 3.90% fewer; `canada` encode 0.78%, decode 5.27% fewer; `semanticscholar` encode 2.73%, decode 16.68% fewer; `twitter.toon` 16.06%, `citm_catalog.toon` 9.83%, `openapi_github.toon` 15.11% fewer (decode). Every cell counted is fewer
- Why kept although the guard failed: the guard failed on code the lever does not touch, and the stack is fewer than its base on all eleven cells; this is the author's judgement against the author's own guard, stated here; revert `convert.mode`'s encode arm to undo it (NE-035's encode cost, bead `toon_bend-1yf`, comes back with it)
- Binding: the quantified law that the fused walk IS the two walks (withdrawn with the code on 2026-09-24) (the fused walk IS the two walks, for every input, decoder state and reader state; by induction on the bytes with a split on the reader's mode) and five closed goldens from the pinned original (NE-034); the full proof: `All terms check.` over 594 laws at that commit, 682 s, 5.2 GB
- Correctness evidence: conform c-1t 1076/1076 with `TOON_SPEC` unset and set
- **REVERTED 2026-09-24, the reason that decides it:** the four-lane run on the push candidate (`scripts/lanes.sh … --threads 8`) FAILED on the interpreter and js lanes, 1074/1076 each, c-1t and c-8t PASS: `jsonerr_utf8_surrogate` and `jsonerr_utf8_above_max` printed `bend: 55296 is not a Unicode scalar value` (resp. 1114112) instead of the UTF-8 message. The fused walk runs the JSON reader on bytes whose UTF-8 verdict is not known yet; on `ED A0 80` the reader completes the scalar U+D800 and builds a `Char` from it, and the JavaScript runtime (which the interpreter lane also runs on) refuses a Char that is not a Unicode scalar value, where the C runtime does not check. The card had argued the reader was safe on invalid input because the checker proves it total: the checker's totality says nothing about a runtime's value checks. That law was true and is withdrawn with the code it spoke about; the five closed goldens stay (they hold for EXP-031's code, which decides the verdict before the reader reads anything). The lever's conform run was c-1t only (twice, with and without `TOON_SPEC=1`); the js and interpreter lanes were not run before it was kept, which is the process failure behind this entry
- **Do-not-retry unless:** the fused walk stops before the reader completes a scalar the decoder rejects (for example, it returns as soon as the decoder's state becomes Invalid, since the verdict is then decided), its law is restated for that shape, and the js and interpreter lanes pass `jsonerr_utf8_*` BEFORE any count is taken
- Tally: W0/L1/N0 (reverted: a lane difference)
- Agent: Claude (session ef481f9c, 2026-09-23; reverted 2026-09-24)

### NE-037 — path expansion's two walks fused into one (EXP-033)   [2026-09-24 | NEGATIVE(not built), withdrawn]

- Program / def: `port/decode.bend` / `obj.fin` (S3.28) `x.json(x.norm(x))`, with `x.norm` (S3.28) and `x.json` (S3.28)
- Provenance: no build, no binary, no capture. `port/` was never modified. Carded and withdrawn the same day from reading four defs of `port/decode.bend` at the frozen tree (the BINARY is identical across `9c9039e`, `55ec958` and `c5f3b46`, `sha256 825e44de69c3a4e6c0d442ea`; the tree is not — `55ec958`→`c5f3b46` changes `LAWS.bend`, `PROOF.bend` and three comment lines in `json.bend`, none reached by `main.bend`'s imports or by codegen)
- What was carded: `obj.fin` to call one fused `x.json.norm(x: XV) -> J.Json` in place of `x.json(x.norm(x))`, behind `F.twin.on`, bound by a QUANTIFIED `for x: XV { x.json.norm(x) == x.json(x.norm(x)) }` law (reachable here, unlike the numeric twins of NE-001..003, because both sides are structural walks with no `Nat` arithmetic). Gate precommitted in instructions: ≥ 15% fewer on `citm_catalog.fold.toon --decode --expand-paths safe`
- **Why it was withdrawn, finding 1:** the card's hypothesis said `x.json` walks the normalised tree and discards it. It does not — `x.json` is a two-arm match on one constructor (`XLeaf{v}` → `v`, `other` → `JNull{}`), with no recursion, and is too small to appear anywhere in the profile. The lever removes ONE DISPATCH per finished object, not a traversal, so a 15% gate was never reachable
- **Why it was withdrawn, finding 2:** the obvious stronger variant — drop `x.norm(map)` and let `x.entries` read the map directly, because a finished object's map values are "already normalised" — is unsound. `xo.set` (S6.42) stores `XObj` values and `lk.of` (S4.244) re-opens an `XLeaf{J.JObj{entries}}` into an `XObj` via `xv.open` (S4.245) for merging, so the map legitimately holds objects still UNDER CONSTRUCTION. Those are exactly what dotted-key expansion creates, and none can be finished earlier because another dotted key may still arrive. `x.norm`'s descent is load-bearing. Fusing normalisation into retrieval instead needs `x.entries` (list recursion) to call back into the `XV` recursion: mutual recursion, forbidden here
- **Two measurement errors corrected with it:** (a) the card cited expansion's own logic as 7.9% of the delta, reading `WL_FID_DECODE_X_NORM` alone, when the compiler hoists a def's match arms into their own worklist entries (`_K998`…`_K1001`) and the base symbol under-counts the def by a third; (b) larger, the profile behind the card was taken WITHOUT `--threads 1`, so the encoder's parallel number pre-pass (EXP-007) made it irreproducible (~6e-5) and inflated it unevenly (a decode by 10.9%). Re-measured at one thread the expansion delta is **605,239,282 (+41.1%)** against 864,715,290 (+53.0%), the port pays **23.7×** the oracle rather than 33.8×, and `WL_FID_EXIT` (25.3% of the old delta) vanishes as parallel worklist machinery. At one thread, grouped by base def: `x.norm` **14.8%**, all `X_*` defs 21.7%, named port defs 25.1%, runtime and `spin_N` 67.7%, the refcount trio 54.3%. The corrected delta cross-validates against THIS ledger's own NE-027, which measured 605,259,269 for the same quantity on an earlier binary — 3e-5 apart. Neither correction changes this entry's verdict, which rests on reading the defs
- Killing metric: none was needed — the lever's ceiling was bounded by reading the code below its gate
- **What this adds to NE-027's rule.** NE-027 ("eliminate a pass over the data, not an operation within one") is why this lever was aimed at a pass. The converse is now on the record too: **check the pass is REMOVABLE before carding it.** Each of expansion's three phases (`expand` builds the `XV`, `x.norm` finishes the objects under construction, `x.entries` reads the map back in insertion order) is load-bearing, so this path has no pass to remove — and NE-027 already showed that an operation lever on it counts 0.11% against a 5% gate. The remaining attributable cost is refcount traffic over a `+`-shared `XV`, which is `toon_bend-0uw` and is governed by NE-012's do-not-retry
- **Do-not-retry unless:** a finished object's map can no longer hold an `XObj` (both `xo.set`'s `XObj` values and `lk.of`'s re-opening gone), OR Bend admits the mutual recursion that normalise-on-retrieval needs
- Tally: W0/L0/N1 (withdrawn before build; nothing was measured, so nothing is claimed)
- Agent: Claude (session 9e91730b, 2026-09-24)

### NE-038 — a wall-clock startup ratio against the original (`INCUMBENT.startup`)   [2026-09-25 | NO_EVIDENCE, not obtainable by this measurement design]

- Program / def: none. The arm is `--version` on both binaries: the port's `port/main.bend` shell reaching `IO.print`, and the original's clap version path
- Provenance: `perf/evidence/INCUMBENT.startup.json`, REFUSED_CV, 30 samples per arm, `oracle/toon` (then `f955c67`) median **0.96 ms at cv 31.1%** against the port at **0.95 ms at cv 19.8%**. `docs/PORT_REPORT.md` recorded this refusal, with four others, as the host's ("Owner: the author, on a quiet host")
- **Why no quiet host fixes it, and why it is not the same as the other four refusals.** The other four are input-bound: their ORIGINAL arm was 3.45–5.92 ms, and scaling the input the way `tabular_30000.json` was scaled puts it above the 100 ms arm predicate that `toon_bend-udw` (closed) and `toon_bend-0i8` recorded. **`--version` has no input to scale.** At ~1 ms an arm is one `fork`/`exec`, the dynamic loader, and one scheduler placement; that spread is the quantity the cv gate reads, and it does not shrink with a quieter host the way a 100 ms arm's does. Nor does it shrink with samples: more samples narrow the standard error of the MEDIAN, while `cv` is the spread of the sample distribution itself, which is what `scripts/incumbent-bench.sh` gates on. So this refusal is a statement about the design of the measurement, not about the machine, and leaving it as "retry on a quiet host" would leave a permanent false hope in the register
- Killing metric: none was needed. The two arm medians, 0.96 ms and 0.95 ms, are themselves the argument: they are three orders of magnitude below the arm length the gate's 5% threshold was calibrated for
- **What already exists for the same quantity, in a different claim class.** `perf/COUNTED-PROFILE.md`'s `startup_encode` row: port **532,601** instructions against the oracle's **504,653**, a ratio of **1.06×** on a 7-byte input, counted with cachegrind at `--threads 1`. Counted evidence is deterministic and load-insensitive, so it needed no quiet host at all. It is NOT a wall-clock claim and is not offered as one: it says the port executes 1.06× the instructions to start and encode 7 bytes, not that it takes 1.06× the time
- **Do-not-retry unless** the arm is REDEFINED so that one sample is N sequential invocations (200 startups per sample puts the arm near 190 ms on these medians), which needs its own card because every published per-invocation row would then be measuring a different quantity from this one; OR a timer whose per-invocation resolution is below process-spawn jitter becomes available on this host
- Tally: W0/L0/N1 (nothing was measured, so nothing is claimed)
- Agent: Claude (session 9e91730b, 2026-09-25)
