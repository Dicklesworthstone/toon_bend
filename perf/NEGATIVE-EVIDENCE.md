...
### NE-036 — on top of EXP-031, encoding reads the input bytes once (EXP-032, NE-034's fused walk retried)   [2026-09-23 | COUNTED_WIN on the primary gate, decode guard FAILED, kept]
...
- The decode path's Bend code is the same in both builds (the lever adds two types and four defs to `json.bend` and changes only the encode arm); the constructor table of the emitted C changes with the two new types. The cause of the decode difference is NOT identified. A finding for every COUNTED entry of this port: two builds whose constructor tables differ can differ by up to about 2% on code neither lever touched, so a counted difference below 2% between such builds is not attributable to a def without a per-function comparison
...
- Correctness evidence: conform c-1t 1076/1076 with `TOON_SPEC` unset and set
- **Do-not-retry unless:** (a win, kept) — a per-function comparison of the decode cells names what moved; revert only if a cv-gated CPU capture shows the stack slower on decode than its base
- Tally: W1/L0/N0 (counted, guard failed)
- Agent: Claude (session ef481f9c, 2026-09-23)
