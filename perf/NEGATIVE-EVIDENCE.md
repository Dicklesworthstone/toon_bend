...
- Exact command: `scripts/incumbent-bench.sh --runs 18 --max-cv 5 --tag EXP-005 --original <baseline binary> --threads 1 -- -e perf/inputs/sci_5000.json --port <lever binary> --threads 1 -- -e perf/inputs/sci_5000.json` (`perf/evidence/EXP-005.ab.json`; `--runs 18` is read back from the JSON's `samples`, one sample per arm per pair at `scripts/incumbent-bench.sh:80-95`, since the script records no run count. Both arms are builds of THIS port, so the entry says nothing about the original)
...
- What the round taught, which is the reusable part: `BN.mul_small` is 39% of all def CALLS on this input, nearly all under `BN.pow10` (the card's third profile), and replacing about three quarters of those multiplications moved the median by about 8%. A call census is not a wall census on this program: the big-natural power is built once per number, while the digit generator runs per digit (EXP-006's profile attributes 75% of def calls on the doubles input to `BN.cmp`, `BN.sub`, `BN.is_ge`, `BN.is_lt` and `F.dg.*`)
