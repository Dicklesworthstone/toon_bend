...
- Provenance: as NE-022; base = the binary of the EXP-029 code (`faaccac`), `--threads 1`
...
- Program / def: `port/cli.bend` / `convert` decides the mode first (`convert.mode`, `o.decode`, `o.stats`); decode reads the text alone (`convert.text` no longer takes the bytes); encode is `faaccac`'s path under a new name (`convert.enc`). Not behind the switch: no value changes, only which def holds the bytes
- Provenance: as NE-022; base = the binary of `faaccac` (the EXP-029 code), `--threads 1`
