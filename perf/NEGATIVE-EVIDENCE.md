...
- **A measurement error corrected with it:** the card cited expansion's own logic as 7.9% of the +864,715,290-instruction delta, reading `WL_FID_DECODE_X_NORM` alone. The compiler hoists a def's match arms into their own worklist entries (`_K998`…`_K1001`), so the base symbol under-counts the def by a third. Grouped by base def: `x.norm` **12.5%**, all `X_*` defs 18.3%, all named port defs 21.4%, runtime primitives and `spin_N` **73.0%**. `perf/COUNTED-PROFILE.md` carries the corrected table; the uncorrected one was never cited outside it
