...
No row is admitted yet (2026-09-20). Round 10 (non-author) read this ledger against its own admission rules: the three
EXP-004 rows that stood here had a baseline of ONE timed run (no cv, no A/A arm: rule 2 refuses them), an evidence
directory without the emitted C (rule 7), and an empty `cpu` field in every fingerprint (rule 6; `scripts/incumbent-bench.sh`
reads the model name from `/proc/cpuinfo` since). They are `perf/NEGATIVE-EVIDENCE.md` NE-005 now, PROVISIONAL, with
what would admit them. The three number levers are NE-001..003, the refused folding capture NE-004.
